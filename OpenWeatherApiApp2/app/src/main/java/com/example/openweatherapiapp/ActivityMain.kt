...
class ActivityMain : AppCompatActivity() {
    ...
    val LAT: String = "42.24"
    val LONG: String = "88.24"
    val API: String = "efbd206701b07066d7219906279dc109\n"
    ...
    private fun getResultFromAPI(): String{

        //make the URL CALL
        try{
            val myURL = URL("https://api.openweathermap.org/data/3.0/onecall?lat={$LAT}&lon={$LONG}&appid={$API}&units=imperial").readText(
                Charsets.UTF_8
            )
            response = myURL
    ...
    private suspend fun populateInfo(incomingJSON: String){
        withContext(Main){
            ...
            val temp = main.getString("current.temp")+"°F"
